...
def patch_cdf(x,y, extra_parameters = None):
    if extra_parameters is None:
        ...
        #calculate the area of the cicular patch
        area = np.sqrt((x - center_x) ** 2 + (y - center_y) ** 2) 
    return (1/area) * is_in_patch(x, y, patch_type, center_x, center_y, side_length)
...
def set_up_initial_condition(X0, X0_distribution, space_points, space_start, space_end, extra_parameters):
    ...
    for x_coordinate in x:
        for y_coordinate in x:
            ...
            initial_condition_array[array_coordinate] = set_value(x_real, y_real, X0/( space_points ** 2 ), X0_distribution, extra_parameters)
